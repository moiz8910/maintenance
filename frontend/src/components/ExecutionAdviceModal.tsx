...
interface ExecutionAdviceModalProps {
  workOrderId: string;
  onClose: () => void;
}

const ExecutionAdviceModal: React.FC<ExecutionAdviceModalProps> = ({ workOrderId, onClose }) => {
  ...
  useEffect(() => {
    const fetchAdvice = async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/work-order/${workOrderId}/execution-advice`, { method: 'POST' });
        const json = await res.json();
        setAdvice(json.advice);
        setData(json);
      } catch (error) {
        setAdvice("Failed to generate AI execution advice. Please try again.");
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="relative w-full max-w-4xl bg-white rounded-[32px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
        ...
        <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-white relative z-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-900 tracking-tight">Execution Advice</h2>
              <div className="flex items-center gap-2 mt-0.5">
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Work Order: {workOrderId}</p>
                <span className="text-[10px] text-slate-300">•</span>
                <p className="text-[10px] text-indigo-600 font-bold uppercase tracking-widest">Asset ID: {data?.asset_id || '—'}</p>
              </div>
            </div>
          </div>
...
export default ExecutionAdviceModal;
